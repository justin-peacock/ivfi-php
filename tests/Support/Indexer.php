...
final class Indexer
{
    ...
    /**
     * Whether a CGI binary is available for tests that need response headers.
     */
    public static function cgiBinary(): ?string
    {
        ...
        foreach (['php-cgi', 'php-cgi8.4', 'php-cgi8.3'] as $candidate) {
    ...
    /**
     * Renders through CGI, which is how PHP runs behind nginx and, unlike the
     * CLI, actually emits response headers. Needed wherever a test has to read
     * what the server sent back rather than only what it printed.
     *
     * @param array<string, string> $server Extra CGI environment values
     */
    public static function renderCgi(
        Fixture $fixture,
        string $uri = '/',
        array $server = []
    ): Response {
        $binary = self::cgiBinary();

        if ($binary === null) {
            throw new \RuntimeException('No CGI binary available');
        }
        ...
        $env = array_merge([
            ...
            'SERVER_PROTOCOL' => 'HTTP/1.1',
            'SERVER_NAME'     => 'indexer.test',
            'SCRIPT_FILENAME' => $fixture->root() . '/indexer.php',
        ], $server);
        ...
        if (!is_resource($process)) {
            throw new \RuntimeException('Could not start the CGI process');
        }
        ...
        return new Response($stdout, $stderr, $status, true);
    }
}
